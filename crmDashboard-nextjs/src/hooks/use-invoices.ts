...
export function useInvoices() {
  ...
  const loadInvoices = useCallback(async () => {
    try {
      ...
      // Backend returns snake_case format, convert to frontend format with both formats
      const converted = backendInvoices.map((inv) => {
        // If it already has camelCase fields, it's already converted
        if ((inv as any).invoiceNumber || (inv as any).invoice_number) {
          ...
          return {
            ...
            amount: invoice.amount || invoice.subtotal,
            totalAmount: invoice.totalAmount || invoice.total,
            items: invoice.items || invoice.line_items?.map((item: any) => ({
              id: item.id,
  ...
  const updateInvoice = async (invoiceId: string, updates: Partial<Invoice>) => {
    try {
      const updatedInvoice = await invoicesService.updateInvoice(invoiceId, {
        status: updates.status,
        due_date: updates.due_date || updates.dueDate,
        paid_date: updates.paid_date || updates.paidDate,
        notes: updates.notes,
      });
      // Update the invoice in state directly instead of reloading all
      setInvoicesList((prev) => {
        const converted = backendInvoiceToFrontend(updatedInvoice as unknown as BackendInvoice);
        return prev.map((inv) => (inv.id === invoiceId ? converted : inv));
      });
